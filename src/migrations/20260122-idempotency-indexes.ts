...
export const idempotencyIndexesMigration: IMigration = {
  start: async () => {
    ...
    try {
      ...
      logger.info(
        'Creating compound unique index on proposals collection',
        llo({
      ...
      await db
        ...
        .createIndex({ network: 1, blockHash: 1, logIndex: 1, eventType: 1 }, { unique: true, sparse: true })

      logger.info('Successfully created index on proposals', llo({}))

      // Add compound unique index for Vote idempotency
      logger.info(
        'Creating compound unique index on votes collection',
        llo({
      ...
      await db
        .collection('votes')
        .createIndex({ network: 1, blockHash: 1, logIndex: 1 }, { unique: true, sparse: true })
      ...
      const proposalUpdateResult = await db
        .collection('proposals')
        .updateMany({ blockHash: { $exists: false } }, { $set: { blockHash: null } })
      logger.info(
        'Backfill complete for proposals',
        llo({
      ...
      const voteUpdateResult = await db
        .collection('votes')
        .updateMany({ blockHash: { $exists: false } }, { $set: { blockHash: null } })
      logger.info(
        'Backfill complete for votes',
        llo({
